print("""If we take 47, reverse and add, 47 + 74 = 121, which is palindromic.
...
def check_Lycheral(num, depth = 50):
    ...
    if isPalindrome(result):
        ...
        if depth > 0:
            not_Lycheral, ls = check_Lycheral(result, depth - 1)
            if not_Lycheral:
                return not_Lycheral, ls + [num]
            else:
                return False, [] 
        else:
            return False, []
...
for i in range(1,10001):
    print(i)
    if i in non_Lychreal:
        continue

    not_Ly, non_Ly_ls = check_Lycheral(i)
    if not not_Ly:
        ans += 1 
    else:
        non_Lychreal += non_Ly_ls

print("Answer = ", ans)

# print(check_Lycheral(1292))